...
public:
    ...
    Section();
    Section(int sec_id, string timeslotID, string courseID, string location, 
            double credits, string reamarks, string weeks, string model, string teacher_username, int maxnum);
};

Section::Section(){}

Section::Section(int t_sec_id, string t_timeslotID, string t_courseID, string t_location, 
...
int main()
{
    ...
    table_name = " section ";
    filed = " ( secId, semester, year, timeslotId, courseId, teacherUserName, location, credits, weeks, remarks, model, maxNum, currentNum ) ";
    for (int i = 0; i < section_size; ++i){
        values[0] = "\"" + to_string(section[i].secID) + "\"";
        values[1] = "\"" + section[i].semester +"\"";
        values[2] = "\"" + section[i].year +"\"";
        values[3] = "\"" + section[i].timeslotID + "\"";
        values[4] = "\"" + section[i].courseID + "\"";
        values[5] = "\"" + section[i].teacher_username + "\"";
        values[6] = "\"" + section[i].location + "\"";
        values[7] = to_string(section[i].credits);
        values[8] = "\"" + section[i].weeks +"\"";
        values[9] = "\"" + section[i].reamarks +"\"";
        values[10] = "\"" + section[i].model +"\"";
        values[11] = to_string(section[i].maxnum);
        values[12] = to_string(section[i].currentnum);
    ...
    values = new string[value_size];
    table_name = " course ";
    filed = " ( courseId, courseName, courseType, electiveType ) ";
    for(int i = 0; i < section_size; ++i){
